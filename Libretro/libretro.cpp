...
extern "C" {
	...
	RETRO_API void retro_set_environment(retro_environment_t env)
	{
		...
		static constexpr struct retro_variable vars[] = {
			...
			{ MesenRamState, "Default power-on state for RAM; All 00|All FF|Random Values" },
	...
	void update_settings()
	{
		...
		if(readVariable(MesenPalette, var)) {
			...
			} else if(value == "Smooth V2 (by FirebrandX)") {
				_console->GetSettings()->SetUserRgbPalette(smoothPalette);
			} else if(value == "Digital Prime (by FirebrandX)") {
				_console->GetSettings()->SetUserRgbPalette(digitalPrimePalette);
			...
			} else if(value == "Nestopia") {
				_console->GetSettings()->SetUserRgbPalette(nestopiaPalette);
			} else if(value == "RockNES") {
				_console->GetSettings()->SetUserRgbPalette(rockNESPalette);
			} else if(value == "Nintendulator") {
				_console->GetSettings()->SetUserRgbPalette(nintendulatorPalette);
			} else if(value == "Sony CXA2025AS") {
				_console->GetSettings()->SetUserRgbPalette(sonyCXAPalette);
			} else if(value == "Zero Degree YUV") {
				_console->GetSettings()->SetUserRgbPalette(zeroDegreePalette);
